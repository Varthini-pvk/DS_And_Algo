...
public class P055_SubStringCharacter2 {
...
//	}
	...
	private String checkSubString1(String inputOne, String inputTwo)
	{
	...
	private int checkSubString(String haystack, String needle)
	{
		{
			if(needle.length()==0) return 0;
			if(haystack.length()>=needle.length())
			{
				int start=0;int end=needle.length(); int count=end;
				int[] refArray=new int[26];
				for(int i=0;i<end;i++)
				{
					refArray[needle.charAt(i)-'a']++;

				}
				for(int i=0;i<end;i++)
				{
					if(refArray[haystack.charAt(i)-'a']>0) 
					
						count--;
						refArray[haystack.charAt(i)-'a']--;
				...
				while(end<haystack.length())
				{
					...
					if(refArray[haystack.charAt(start)-'a']>=0) 
					
						count++;
						refArray[haystack.charAt(start)-'a']++;
					
					start++;
					
					if(refArray[haystack.charAt(end)-'a']>0) 
					
						count--;
						refArray[haystack.charAt(end)-'a']--;
